...
const Index = () => {
  useEffect(() => {
    ...
    const cursor = isTouch ? null : document.createElement("div");
    ...
    const hoverables = Array.from(
      document.querySelectorAll('[data-cursor="nav-link"]'),
    ) as HTMLElement[];
    ...
    magnets.forEach((m) => {
      m.addEventListener("mousemove", onMag);
    ...
    return () => {
      ...
      magnets.forEach((m) => {
        m.removeEventListener("mousemove", onMag);
